...
async function audit() {
  const { data, error } = await sb.from('personal_asignaciones_jornada').select('*').order('fecha_inicio', { ascending: true });
  const byPerson = {};
  ...
  for (const [personalId, rows] of Object.entries(byPerson)) {
    for (let i = 1; i < rows.length; i++) {
      ...
      if (sameCycle) {
        ...
        if (diffDays % cycleLen !== 0) { invalidResets++; workersWithInvalidReset.add(personalId); }
      } else {
        validChanges++; workersWithValidChange.add(personalId);
      }
    }
  }
