-- Reconoce contratos del catalogo por sus metadatos visibles, no solo por
-- captura_snapshot_laboral o por el texto tecnico almacenado en tipo_doc.
    ...
    FROM (
      ...
      WHERE d.empresa_id = p_empresa_id
        ...
        AND (
          ...
          OR lower(COALESCE(td.categoria, '')) = 'contractual'
          OR lower(concat_ws(' ', td.codigo, td.nombre, td.id, d.tipo_doc)) LIKE '%contrato%'
          OR lower(concat_ws(' ', td.codigo, td.nombre, td.id, d.tipo_doc)) LIKE '%adenda%'
        )
    ) x
