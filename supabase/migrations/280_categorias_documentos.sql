-- ============================================================================
-- 280 · Categorías de Tipos de Documento
-- ============================================================================
...
-- 2. Backfill de categorías a los tipos existentes
UPDATE public.tipos_documento_empresa
SET categoria = CASE
  WHEN codigo IN ('DOC-PRIM', 'DOC-001', 'DOC-ADENDA') THEN 'Contractual'
  WHEN codigo IN ('TDOC-007', 'TDOC-009', 'TDOC-004') THEN 'Identificación'
  WHEN codigo IN ('TDOC-001', 'TDOC-002', 'TDOC-003') THEN 'Seguridad y Salud'
  WHEN codigo IN ('TDOC-005', 'TDOC-006') THEN 'Capacitación'
  WHEN nombre ILIKE '%contrato%' OR nombre ILIKE '%adenda%' OR nombre ILIKE '%convenio%' THEN 'Contractual'
  WHEN nombre ILIKE '%dni%' OR nombre ILIKE '%carnet%' OR nombre ILIKE '%pasaporte%' THEN 'Identificación'
  WHEN nombre ILIKE '%seguro%' OR nombre ILIKE '%sctr%' OR nombre ILIKE '%salud%' OR nombre ILIKE '%médico%' OR nombre ILIKE '%epp%' THEN 'Seguridad y Salud'
  WHEN nombre ILIKE '%certificad%' OR nombre ILIKE '%capacitaci%' OR nombre ILIKE '%curso%' THEN 'Capacitación'
  ELSE 'Otros'
END
WHERE categoria = 'Otros' OR categoria IS NULL;

-- 3. Reflejar esto en la vista si es necesario (estado_global_colaboradores no expone la categoría, pero calcular_habilitaciones_personal sí debería)
...
RETURNS TABLE (
  ...
  tipo_sucesor_id      text,
  documento_padre_tipo_id text
)
LANGUAGE sql
STABLE
SECURITY DEFINER
...
AS $$
  ...
  estado_docs AS (
    SELECT
      ...
      tde.categoria     AS categoria,
      tde.es_habilitante,
      tde.requiere_validacion,
      tde.exige_vencimiento,
      tde.dias_alerta,
      tde.tipo_sucesor_id,
      tde.documento_padre_tipo_id,
      cdr.obligatorio,
      d.estado_validacion,
      d.fecha_vencimiento,
      d.es_indefinido,
  ...
  SELECT
    ...
    CASE
      ...
      WHEN es_indefinido                                                         THEN 'vigente'
    ...
    fecha_vencimiento,
    CASE
      WHEN exige_vencimiento AND fecha_vencimiento IS NOT NULL AND NOT COALESCE(es_indefinido, false)
...
-- 4. Recrear vista (si es necesario que refleje la nueva estructura, aunque no estamos cambiando los tipos devueltos, solo el origen).
