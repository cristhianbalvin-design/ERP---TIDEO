-- ============================================================================
-- 261 · Contrato Primigenio y Firma de Trabajador
-- ============================================================================

-- 1a. Agregar campo a tipos_documento_empresa
ALTER TABLE public.tipos_documento_empresa 
ADD COLUMN IF NOT EXISTS permite_firma_trabajador boolean DEFAULT false;

-- 1b. Insertar el nuevo tipo "Contrato Primigenio" para todos los tenants que ya tienen "Contrato Laboral"
DO $$
DECLARE
    v_empresa_id text;
    v_next_codigo text;
...
BEGIN
    FOR v_empresa_id IN 
        ...
        WHERE nombre ILIKE '%Contrato Laboral%' OR codigo = 'DOC001'
    LOOP
        -- Generar ID
        v_new_id := 'tdoc_' || replace(gen_random_uuid()::text, '-', '');
        v_new_id := substring(v_new_id from 1 for 23);

        -- Obtener ambito del contrato laboral existente
        SELECT ambito INTO v_ambito 
        FROM public.tipos_documento_empresa 
        WHERE empresa_id = v_empresa_id AND (nombre ILIKE '%Contrato Laboral%' OR codigo = 'DOC001')
        LIMIT 1;

        -- Determinar siguiente código disponible
        v_next_codigo := 'DOC-PRIM';

        INSERT INTO public.tipos_documento_empresa (
            id, empresa_id, codigo, nombre, ambito, 
            exige_vencimiento, dias_alerta, es_habilitante, 
            requiere_validacion, captura_snapshot_laboral, 
            renovable, permite_firma_trabajador, documento_padre_tipo_id, estado
        ) VALUES (
            v_new_id, v_empresa_id, v_next_codigo, 'Contrato Primigenio', coalesce(v_ambito, 'personal'),
            true, 7, true,
            true, true,
            false, false, null, 'activo'
        ) ON CONFLICT DO NOTHING;
    END LOOP;
END;
$$;

-- 1c. Actualizar el tipo "Contrato Laboral" existente para reflejar su rol correcto
UPDATE public.tipos_documento_empresa
SET permite_firma_trabajador = true
WHERE nombre ILIKE '%Contrato Laboral%' OR codigo = 'DOC001';


-- Modificamos subir_version_documento para aceptar p_origen y verificar permite_firma_trabajador
...
BEGIN
  ...
  IF p_origen = 'portal_empleado' THEN
    ...
    -- NUEVA VALIDACIÓN: Verifica si el tipo de documento permite firma por parte del trabajador
  ...
  -- Obtenemos renovable desde tipos_documento_empresa
