-- Vinculo opcional CxP -> orden de compra y bloqueo de CxP duplicada por OC.
-- importar_cxp_masiva_fila mantiene su ruta independiente y no se modifica
-- en este esfuerzo.
...
begin
  if not exists (
    ...
    where conrelid = 'public.cxp'::regclass
      and conname = 'cxp_orden_compra_id_fkey'
  ) then
    alter table public.cxp
      add constraint cxp_orden_compra_id_fkey
      foreign key (orden_compra_id)
      references public.ordenes_compra(id)
      on delete set null;
  end if;
end;
$$;

create index if not exists idx_cxp_orden_compra_id
  on public.cxp(orden_compra_id)
...
declare
  ...
  v_cxp_existente public.cxp%rowtype;
...
begin
  ...
  if v_orden_compra_id is not null then
    ...
    select * into v_cxp_existente
    from public.cxp
    where empresa_id = v_empresa_id
      and orden_compra_id = v_orden_compra_id
    order by created_at desc nulls last
    limit 1;

    if found then
      if lower(coalesce(v_cxp_existente.estado, '')) = 'pagada'
         or (coalesce(v_cxp_existente.monto_total, 0) > 0
             and coalesce(v_cxp_existente.monto_pagado, 0) >= v_cxp_existente.monto_total) then
        raise exception 'Esta orden de compra ya tiene una CxP registrada y pagada.';
      end if;
      raise exception 'Esta orden de compra ya tiene una CxP registrada, pendiente de pago.';
    end if;
  end if;
