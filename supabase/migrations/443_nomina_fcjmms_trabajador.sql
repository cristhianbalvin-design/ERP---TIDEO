-- TIDEO ERP - FCJMMS trabajador (Ley 29741) en el snapshot de nómina.
-- Es una retención del trabajador; no representa el aporte anual del empleador.

alter table public.nomina_detalle
  add column if not exists fcjmms_trabajador numeric(14,2) not null default 0;

-- La implementación legacy fue renombrada por la migración 414; conserva el
-- mapeo explícito de JSONB para que la nueva columna se persista.
...
begin
  ...
  insert into public.nomina_detalle (
    ...
    dias_laborables, dias_laborados, dias_computables,
    horas_extra_tramo1_min, horas_extra_tramo2_min,
    sueldo_base, remuneracion_bruta, asignacion_familiar, add_horas_extra,
    bonif_altitud, otros_ingresos, desc_faltas, desc_tardanzas,
    aporte_afp, comision_afp_flujo, prima_seguro_afp, desc_onp, fcjmms_trabajador,
