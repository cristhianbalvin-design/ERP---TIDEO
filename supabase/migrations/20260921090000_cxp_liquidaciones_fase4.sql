-- Fase 4: habilita la anulacion de CxP de Liquidaciones por Cese
-- a traves del mismo autorizador centralizado.
...
begin
  ...
  v_permitido := case v_origen
    ...
    when 'nc_devolucion' then public.usuario_puede(v_empresa_id, 'cxp', 'crear')
    ...
    when 'gasto_movil' then public.usuario_es_superadmin_plataforma()
      or exists (
        select 1 from public.usuarios_empresas ue
        where ue.user_id = auth.uid() and ue.empresa_id = v_empresa_id
          and ue.estado = 'activo' and 'compras' = any(coalesce(ue.campo_modulos, array[]::text[]))
      )
    when 'comisiones_rhe' then public.usuario_puede(v_empresa_id, 'cxp', 'crear')
  ...
  if v_origen in ('recepcion_create', 'recepcion_complete') and v_recepcion_id is not null then
    if exists (select 1 from public.cxp where empresa_id = v_empresa_id and recepcion_id = v_recepcion_id) then
  ...
  if v_origen = 'recepcion_complete' then
    ...
    if nullif(btrim(coalesce(v_payload ->> 'factura_proveedor_numero', '')), '') is null then
      raise exception 'El numero de factura del proveedor es obligatorio para completar la recepcion';
    end if;
    ...
    where id = v_recepcion_id and empresa_id = v_empresa_id;
  ...
  if v_cxp.sociedad_id is not null and v_alcance is not null and not (v_cxp.sociedad_id = any(v_alcance)) then
