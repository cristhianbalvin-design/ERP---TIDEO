-- ============================================================================
-- 398 · Bloquear huecos de asignación de jornada hacia el futuro
-- ============================================================================
-- Al crear una asignación de jornada, si el trabajador no tiene un tramo
-- vigente (fecha_fin IS NULL), la nueva fecha_inicio debe obligatoriamente
-- calzar de forma continua: o bien es el día siguiente a su último tramo
-- cerrado, o es su fecha_ingreso. Esto evita huecos silenciosos.
...
DECLARE
  ...
  v_asig_activa_id text;
  v_ultimo_fin date;
  v_fecha_ingreso date;
  v_expected_inicio date;
BEGIN
  ...
  -- ── VALIDACIÓN DE CONTINUIDAD (NO HUECOS) ──────────────────────────────────
  SELECT id INTO v_asig_activa_id
  ...
  IF v_asig_activa_id IS NULL THEN
    SELECT fecha_fin INTO v_ultimo_fin
    FROM public.personal_asignaciones_jornada
    WHERE empresa_id = p_empresa_id
      AND personal_id = p_personal_id
      AND personal_tipo = p_personal_tipo
    ...
    IF v_ultimo_fin IS NOT NULL THEN
      v_expected_inicio := v_ultimo_fin + 1;
    ELSE
      IF p_personal_tipo = 'operativo' THEN
        SELECT fecha_ingreso INTO v_fecha_ingreso FROM public.personal_operativo WHERE id = p_personal_id;
      ELSE
        SELECT fecha_ingreso INTO v_fecha_ingreso FROM public.personal_administrativo WHERE id = p_personal_id;
      END IF;
      v_expected_inicio := v_fecha_ingreso;
    END IF;

    IF v_expected_inicio IS NOT NULL AND p_fecha_inicio <> v_expected_inicio THEN
      RAISE EXCEPTION 'JORNADA_HUECO_INVALIDO: Al no tener un tramo vigente, la fecha de inicio debe ser obligatoriamente % para mantener la continuidad.', v_expected_inicio;
    END IF;
  END IF;
  -- ─────────────────────────────────────────────────────────────────────────────
