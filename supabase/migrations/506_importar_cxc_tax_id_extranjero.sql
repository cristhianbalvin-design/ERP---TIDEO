-- Permite importar CxC para cuentas ya registradas con Tax ID extranjero.
-- Las cuentas creadas desde una plantilla conservan el comportamiento historico: RUC estricto.
...
declare
  ...
  v_identificador_cliente text := nullif(btrim(p_payload ->> 'ruc_cliente'), '');
  v_ruc text := regexp_replace(coalesce(v_identificador_cliente, ''), '\D', '', 'g');
...
begin
  ...
  if v_identificador_cliente is null then
    raise exception 'Documento del cliente obligatorio.';
  end if;
  ...
  -- Serializa la resolucion/creacion del cliente por identificador dentro del tenant.
  perform pg_advisory_xact_lock(hashtext(v_empresa_id || '|CUENTA|' || upper(v_identificador_cliente)));
  select * into v_cuenta
  from public.cuentas
  where empresa_id = v_empresa_id
    and (
      (coalesce(tipo_documento, 'RUC') = 'TAX_ID_EXTRANJERO'
        and upper(btrim(coalesce(ruc, ''))) = upper(v_identificador_cliente))
      or (
        coalesce(tipo_documento, 'RUC') = 'RUC'
        and regexp_replace(coalesce(ruc, ''), '\D', '', 'g') = v_ruc
      )
    )
  order by case
    when coalesce(tipo_documento, 'RUC') = 'TAX_ID_EXTRANJERO'
      and upper(btrim(coalesce(ruc, ''))) = upper(v_identificador_cliente) then 0
    else 1
  end, created_at nulls last, id
  limit 1
  for update;

  if found then
    if coalesce(v_cuenta.tipo_documento, 'RUC') = 'TAX_ID_EXTRANJERO' then
      if char_length(v_identificador_cliente) < 3 or char_length(v_identificador_cliente) > 30 then
        raise exception 'Tax ID extranjero invalido: usa entre 3 y 30 caracteres.';
      end if;
    elsif v_ruc !~ '^\d{11}$' then
    ...
    insert into public.cuentas (
      id, empresa_id, nombre_comercial, razon_social, ruc, tipo_documento, tipo, moneda, estado, saldo_cxc
    ) values (
      v_cuenta_id, v_empresa_id, v_razon_social, v_razon_social, v_ruc, 'RUC', 'cliente', v_moneda, 'activo', 0
  ...
  if v_os_codigo is not null then
    ...
    if v_os.cuenta_id is distinct from v_cuenta.id then
      raise exception 'La OS Cliente indicada no pertenece al cliente del documento cargado.';
    end if;
