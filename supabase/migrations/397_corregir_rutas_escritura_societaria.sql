-- Corrige rutas de escritura societaria sin activar multisociedad ni modificar RLS.
...
ALTER TABLE public.devoluciones_proveedor_lineas
  ADD COLUMN sociedad_id uuid DEFAULT NULL
  REFERENCES public.sociedades(id) ON DELETE SET NULL;

ALTER TABLE public.devoluciones_proveedor
  ADD CONSTRAINT devoluciones_proveedor_empresa_sociedad_fkey
  FOREIGN KEY (empresa_id, sociedad_id)
  REFERENCES public.sociedades(empresa_id, id);

ALTER TABLE public.devoluciones_proveedor_lineas
  ADD CONSTRAINT devoluciones_proveedor_lineas_empresa_sociedad_fkey
  FOREIGN KEY (empresa_id, sociedad_id)
  REFERENCES public.sociedades(empresa_id, id);

CREATE INDEX idx_devoluciones_proveedor_empresa_sociedad
  ON public.devoluciones_proveedor(empresa_id, sociedad_id);

CREATE INDEX idx_devoluciones_proveedor_lineas_empresa_sociedad
  ON public.devoluciones_proveedor_lineas(empresa_id, sociedad_id);

DROP FUNCTION public.crear_financiamiento(
...
begin
  ...
  if v_multisociedad_habilitado then
    if p_sociedad_id is null then
      raise exception 'La sociedad es obligatoria para crear un financiamiento en un tenant con multisociedad.';
    end if;
    select s.id
    into v_sociedad_id
    ...
    where s.id = p_sociedad_id
      ...
      and s.activa = true;
    if not found then
      raise exception 'La sociedad indicada no existe, está inactiva o no pertenece al tenant.';
    end if;
  else
    v_sociedad_id := null;
  end if;
  ...
  insert into public.financiamientos (
    id, empresa_id, sociedad_id, codigo, tipo, entidad, tipo_entidad,
  ...
  values (
    p_id, p_empresa_id, v_sociedad_id, p_codigo, p_tipo, p_entidad, p_tipo_entidad,
...
begin
  ...
  if p_gasto_interes is not null then
    insert into public.compras_gastos (
      id, empresa_id, sociedad_id, tipo, descripcion, categoria, subcategoria, monto, moneda, fecha,
      financiamiento_id, cuota_numero, estado
    )
    values (
      p_gasto_interes ->> 'id', p_empresa_id, v_financiamiento.sociedad_id,
