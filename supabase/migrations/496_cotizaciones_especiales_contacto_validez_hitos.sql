...
-- Sobrecarga con los campos nuevos. La firma histórica de 492 se conserva
-- para no romper callers existentes que aún sólo crean el borrador básico.
...
declare
  ...
  v_hitos jsonb;
begin
  select resultado.id, resultado.numero into v_id, v_numero
  from public.crear_cotizacion_especial(
    p_tipo_documento_id,
  ...
  where cotizaciones_especiales.id = v_id;
...
-- Sobrecarga con datos comerciales para una edición de borrador completa.
-- La firma histórica de dos parámetros de 492 se conserva sin cambios.
create function public.actualizar_items_cotizacion_especial(
  p_id uuid,
  p_items jsonb,
  p_contacto_id text,
  p_validez_tipo text,
...
declare
  ...
  v_hitos jsonb;
begin
  select resultado.id, resultado.items, resultado.subtotal, resultado.igv, resultado.total
    into v_id, v_items, v_subtotal, v_igv, v_total
  from public.actualizar_items_cotizacion_especial(p_id, p_items) resultado;
