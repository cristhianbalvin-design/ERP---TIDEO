-- 519 · Conserva el activo y la recepción de origen en Cotizaciones Especiales.
-- Ejecutar con SET ROLE postgres dentro de una transacción revisada.

alter table public.cotizaciones_especiales
  add column activo_id text references public.activos(id) on delete restrict,
  add column recepcion_id text references public.recepciones_activos_cliente(id) on delete restrict;

-- Se reemplaza la misma firma por una versión con dos argumentos opcionales al final.
-- Los llamados existentes, que no los envían, conservan su comportamiento mediante DEFAULT NULL.
drop function public.crear_cotizacion_especial(
...
begin
  ...
  insert into public.cotizaciones_especiales (
    ...
    p_oportunidad_id, p_hoja_costeo_id, p_activo_id, p_recepcion_id, p_origen_items, v_numero, v_moneda,
