...
BEGIN
  ...
  -- Se toma la última versión de cada período, tanto vigente como archivado.
  -- Un período archivado deja de ser el contrato actual, pero continúa siendo
  -- evidencia válida para las asistencias de las fechas que cubrió.
  contrato_base AS (
  ...
  -- Prioriza el período que cubre p_fecha. Si no hay cobertura, conserva el
  -- período más cercano ya iniciado (o, en último caso, el próximo) para no
  -- alterar el contrato de salida de la función.
  contrato_seleccionado AS (
    SELECT DISTINCT ON (cr.personal_id)
      cr.*
...
'Resuelve la cobertura contractual por la fecha consultada. Incluye períodos
archivados para permitir correcciones históricas, sin reactivarlos; prioriza el
período que contiene p_fecha y conserva el bloqueo para fechas sin cobertura o
