-- Fase 2: recepciones sin factura y completado posterior.
-- La fuente de pendientes es el anti-join recepciones -> cxp por recepcion_id.

create or replace view public.recepciones_pendientes_facturar
with (security_invoker = true)
as
select r.*
from public.recepciones r
left join public.cxp c
  on c.empresa_id = r.empresa_id
 and c.recepcion_id = r.id
where c.id is null;

grant select on public.recepciones_pendientes_facturar to authenticated;
...
begin
  ...
  if v_origen in ('recepcion_create', 'recepcion_complete') and v_recepcion_id is not null then
    if exists (
      select 1 from public.cxp
      where empresa_id = v_empresa_id and recepcion_id = v_recepcion_id
    ) then
  ...
  if v_origen = 'recepcion_complete' then
    if v_recepcion_id is null then
      raise exception 'La recepcion es obligatoria para completar la factura';
    end if;
    ...
    if not found then
      raise exception 'La recepcion % no existe en el tenant indicado', v_recepcion_id;
    end if;
  end if;
