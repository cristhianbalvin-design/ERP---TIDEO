-- Al aprobar un Contrato Primigenio / contrato vigente, esta funcion sincroniza
-- el snapshot de condiciones_laborales hacia personal_operativo / personal_administrativo,
-- pero nunca sincronizaba modalidad_contrato (planilla/honorarios). El formulario de
-- subida de documentos captura ese dato en el mismo campo "Tipo de contrato" (valor
-- 'honorarios'), y el frontend ahora tambien envia condiciones_laborales.modalidad_contrato
-- en el snapshot ('planilla' u 'honorarios'). Esta funcion agrega su sincronizacion,
-- igual que ya se hace con tipo_contrato, cargo, sede, etc.
    ...
    if v_row.personal_tipo = 'operativo' then
      ...
      set cargo             = coalesce(nullif(coalesce(v_cond ->> 'cargo_nombre', v_cond ->> 'cargo'), ''), cargo),
          ...
          tipo_contrato     = case
            ...
            when v_cond ->> 'tipo_contrato' = 'honorarios' then 'por_encargo'
            else tipo_contrato
          end,
          modalidad_contrato = case
            when v_cond ->> 'modalidad_contrato' = any (array['planilla','honorarios'])
              then v_cond ->> 'modalidad_contrato'
            when v_cond ->> 'tipo_contrato' = 'honorarios' then 'honorarios'
            else modalidad_contrato
          end,
