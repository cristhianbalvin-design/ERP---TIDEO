...
serve(async (req) => {
  ...
  const roleAssignment = await assertRoleAssignment(userClient, empresaId, roleRow.id, "crear");
  ...
  if (existingAuthUser?.id) {
    if (modoAutomatico) {
      return jsonResponse({ success: false, error: "Ya existe una cuenta de acceso con este correo. Vincúlala manualmente; la cuenta existente no fue modificada." }, 409);
    }
    alreadyExists = true;
    ...
    if (createError) {
      ...
      if (modoAutomatico) {
        return jsonResponse({ success: false, error: "Ya existe una cuenta de acceso con este correo. La cuenta existente no fue modificada." }, 409);
      }
