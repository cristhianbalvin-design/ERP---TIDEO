-- B0-b - Activacion y backfill societario de UN tenant.
--
-- PROCEDIMIENTO SUPERVISADO. NO ES UNA MIGRACION.
-- Antes de ejecutar:
--   1. Reemplazar REEMPLAZAR_EMPRESA_ID por uno de los seis IDs permitidos.
...
BEGIN
  IF v_empresa_id NOT IN (
    ...
    'emp_20600026446', -- MIC
    'emp_20609996464', -- TIDEO SAC
    'emp_20513453711', -- WHYNCO
    'emp_20541435833'  -- ZAHORY/C&J
  ) THEN
    RAISE EXCEPTION
  ...
  IF v_habilitado IS DISTINCT FROM false THEN
    RAISE EXCEPTION
      'B0B_PRECHECK: el tenant % ya tiene multisociedad_habilitado=%',
      v_empresa_id,
      v_habilitado;
  END IF;

  SELECT count(*) INTO v_sociedades
  FROM public.sociedades
  WHERE empresa_id = v_empresa_id;

  IF v_sociedades <> 0 THEN
    RAISE EXCEPTION
      'B0B_PRECHECK: el tenant % ya tiene % sociedad(es)',
      v_empresa_id,
      v_sociedades;
  END IF;
END;
...
BEGIN
  ...
  WHERE id = v_empresa_id
    AND multisociedad_habilitado = false;

  GET DIAGNOSTICS v_actualizadas = ROW_COUNT;
  IF v_actualizadas <> 1 THEN
    RAISE EXCEPTION
      'B0B_ACTIVACION: se esperaba activar una empresa y se actualizaron %',
      v_actualizadas;
  END IF;
...
-- B4. IDENTIDAD DE LA SOCIEDAD PRINCIPAL
UPDATE public.sociedades s
SET direccion_fiscal = nullif(btrim(ec.direccion), ''),
    logo_url = nullif(btrim(ec.logo_url), ''),
    firma_url = nullif(btrim(ec.firma_url), ''),
    regimen_laboral = coalesce(
      nullif(btrim(ec.regimen_laboral_empresa), ''),
      'general'
    ),
    pct_quincena_1 = coalesce(ec.pct_quincena_1, 50),
    updated_at = now()
FROM public.empresas e
LEFT JOIN public.empresa_config ec ON ec.empresa_id = e.id
WHERE s.id = current_setting('tideo.b0b_sociedad_id')::uuid
