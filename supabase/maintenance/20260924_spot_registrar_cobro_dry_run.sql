...
begin
  ...
  if (ro->'cxc'->>'monto_pagado') is distinct from rn->'cxc'->>'monto_pagado' or (ro->'cxc'->>'saldo') is distinct from rn->'cxc'->>'saldo' or (ro->'cxc'->>'estado') is distinct from rn->'cxc'->>'estado' then raise exception 'CASO_1|parcial|cxc_diferente'; end if;
  if (ro->'cobro'->>'monto_capital') is distinct from rn->'cobro'->>'monto_capital' or (ro->'cobro'->>'medio_pago') is distinct from rn->'cobro'->>'medio_pago' or (ro->'cobro'->>'numero_operacion') is distinct from rn->'cobro'->>'numero_operacion' then raise exception 'CASO_1|parcial|cobro_diferente'; end if;
  if (ro->'movimiento'->>'tipo') is distinct from rn->'movimiento'->>'tipo' or (ro->'movimiento'->>'monto') is distinct from rn->'movimiento'->>'monto' or (ro->'movimiento'->>'moneda') is distinct from rn->'movimiento'->>'moneda' or (ro->'movimiento'->>'cuenta_bancaria_id') is distinct from rn->'movimiento'->>'cuenta_bancaria_id' or (ro->'movimiento'->>'monto_en_moneda_cuenta') is distinct from rn->'movimiento'->>'monto_en_moneda_cuenta' then raise exception 'CASO_1|parcial|movimiento_diferente'; end if;
  if (ro->'comision'->>'monto_cobrado') is distinct from rn->'comision'->>'monto_cobrado' or (ro->'comision'->>'monto_comision') is distinct from rn->'comision'->>'monto_comision' or (ro->'comision'->>'monto_total') is distinct from rn->'comision'->>'monto_total' then raise exception 'CASO_1|parcial|comision_diferente'; end if;
  ...
  if (ro->'cxc'->>'monto_pagado') is distinct from rn->'cxc'->>'monto_pagado' or (ro->'cxc'->>'saldo') is distinct from rn->'cxc'->>'saldo' or (ro->'cxc'->>'estado') is distinct from rn->'cxc'->>'estado' then raise exception 'CASO_1|total|cxc_diferente'; end if;
  if (ro->'cobro'->>'monto_capital') is distinct from rn->'cobro'->>'monto_capital' or (ro->'cobro'->>'numero_operacion') is distinct from rn->'cobro'->>'numero_operacion' then raise exception 'CASO_1|total|cobro_diferente'; end if;
  if (ro->'movimiento'->>'monto') is distinct from rn->'movimiento'->>'monto' or (ro->'movimiento'->>'monto_en_moneda_cuenta') is distinct from rn->'movimiento'->>'monto_en_moneda_cuenta' then raise exception 'CASO_1|total|movimiento_diferente'; end if;
  if (ro->'comision'->>'monto_cobrado') is distinct from rn->'comision'->>'monto_cobrado' or (ro->'comision'->>'monto_comision') is distinct from rn->'comision'->>'monto_comision' or (ro->'comision'->>'monto_total') is distinct from rn->'comision'->>'monto_total' then raise exception 'CASO_1|total|comision_diferente'; end if;
