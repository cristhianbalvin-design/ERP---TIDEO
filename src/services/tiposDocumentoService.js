...
export const tiposDocumentoService = {
  ...
  crearTipoDocumento: async (empresaId, tipo) => {
    ...
    const payload = {
      ...
      ...pick(tipo, ['codigo', 'nombre', 'ambito', 'categoria', 'exige_vencimiento', 'dias_alerta', 'es_habilitante', 'requiere_validacion', 'estado', 'orden', 'captura_snapshot_laboral', 'documento_padre_tipo_id', 'tipo_sucesor_id', 'renovable', 'permite_firma_trabajador']),
    };
    const { data, error } = await supabase
      .from('tipos_documento_empresa')
      .insert([payload])
  ...
  actualizarTipoDocumento: async (id, payload) => {
    ...
    const { data, error } = await supabase
      .from('tipos_documento_empresa')
      .update(pick(payload, ['codigo', 'nombre', 'ambito', 'categoria', 'exige_vencimiento', 'dias_alerta', 'es_habilitante', 'requiere_validacion', 'estado', 'orden', 'captura_snapshot_laboral', 'documento_padre_tipo_id', 'tipo_sucesor_id', 'renovable', 'permite_firma_trabajador']))
      .eq('id', id)
      .select()
      .single();
    if (error) throw error;
