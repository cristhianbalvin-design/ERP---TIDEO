        ...
        <div className="table-wrap"><table className="tbl" style={{ minWidth: 760 }}><thead><tr><th>Recepción</th><th>Código</th><th>Activo</th><th>Ingreso</th><th>Días</th><th>Guía</th><th /></tr></thead><tbody>
          {recepciones.map(recepcion => {
            ...
            return <tr key={recepcion.id}>
              ...
              <td style={{ whiteSpace: 'nowrap' }}><button type="button" className="btn btn-secondary btn-sm" onClick={() => abrirCotizacion(recepcion)} disabled={saving}>Nueva cotización</button><button type="button" className="btn btn-ghost btn-sm" style={{ color: 'var(--danger)' }} onClick={() => abrirDevolucion(recepcion)} disabled={saving}>Devolver sin cotizar</button></td>
            </tr>;
          })}
