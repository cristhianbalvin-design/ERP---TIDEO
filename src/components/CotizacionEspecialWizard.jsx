...
export function CotizacionEspecialWizard({ especialId = null, empresa, empresaConfig, cuentas = [], oportunidades = [], contactos = [], hojasCosteo = [], adaptarHojaCosteo, sociedadIdEscritura, onBack, onCreated, onEmitted }) {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const sb = await getSupabaseClient();
        const { data, error: queryError } = await sb.from('plantillas_documento_bloques').select('*').eq('tipo_documento_id', form.tipo_documento_id).eq('estado', 'publicada').order('version', { ascending:false });
        if (queryError) throw queryError;
