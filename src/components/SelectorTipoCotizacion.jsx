...
export function SelectorTipoCotizacion({ empresaId, onHojaCosteo, onEstandar, onEspecial, onCancel, onError }) {
  ...
  useEffect(() => {
    ...
    listarPlantillasCotizacionPublicadas(empresaId)
      .then(data => {
        if (!activa) return;
        if (!data.length && !onHojaCosteoRef.current) {
