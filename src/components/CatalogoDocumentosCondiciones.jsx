...
const etiquetaTipoDocumento = motorContenido => motorContenido === 'constructor_bloques' ? 'Documento de Bloques' : 'Documento de Condiciones';
...
export function CatalogoDocumentosCondiciones({ active }) {
  ...
  const crearTipo = async () => {
    ...
    try {
      ...
      setNuevoTipo({ codigo:'', nombre:'', motor_contenido:'condiciones_generales' }); setTipos(prev => [...prev, data].sort((a,b) => a.nombre.localeCompare(b.nombre))); setVersionesPorTipo(prev => ({ ...prev, [data.id]:'—' })); setSelectedId(data.id); setVistaActiva('editor');
      setError('');
  ...
  return <div className="params-section params-section-catalogo_documentos" style={{display:'grid', gap:16}}>
    <div className="segmented-control" style={{justifySelf:'start'}}><button type="button" className={`seg-btn ${vistaActiva === 'listado' ? 'active' : ''}`} onClick={() => setVistaActiva('listado')}>Listado</button><button type="button" className={`seg-btn ${vistaActiva === 'editor' ? 'active' : ''}`} onClick={() => setVistaActiva('editor')} disabled={!tipoSeleccionadoReal}>Editor</button></div>
    {vistaActiva === 'listado' && <div className="card"><div className="card-head"><h3>Tipos de documento para cotizaciones</h3>{puedeCrear && <span className="badge badge-cyan">Categoría: cotización</span>}</div><div className="card-body">
      {error && <div className="alert alert-danger">{error}</div>}
      {puedeCrear && <div className="grid-2" style={{gap:8, marginBottom:14}}><input className="input" placeholder="Código, ej. COTIZACION_SERVICIOS" value={nuevoTipo.codigo} onChange={e=>setNuevoTipo(p=>({...p,codigo:e.target.value}))}/><div className="row" style={{gap:8}}><input className="input" placeholder="Nombre del tipo" value={nuevoTipo.nombre} onChange={e=>setNuevoTipo(p=>({...p,nombre:e.target.value}))}/><select className="input" value={nuevoTipo.motor_contenido} onChange={event=>setNuevoTipo(previous=>({...previous,motor_contenido:event.target.value}))} title="Tipo de documento"><option value="condiciones_generales">Documento de Condiciones</option><option value="constructor_bloques">Documento de Bloques</option></select><button className="btn btn-primary" onClick={crearTipo}>Crear tipo</button></div><div className="text-muted" style={{fontSize:12, gridColumn:'1/-1'}}>El valor por defecto conserva el editor actual; selecciona Documento de Bloques antes de crear un tipo especial.</div></div>}
      {loading ? <div className="text-muted">Cargando…</div> : tipos.length === 0 ? <div className="text-muted">No hay tipos configurados para este alcance.</div> : <div className="table-wrap"><table className="tbl"><thead><tr><th>Nombre</th><th>Código</th><th>Tipo</th><th>Default</th><th>Estado</th><th>Versión</th><th></th></tr></thead><tbody>{tipos.map(tipo=><tr key={tipo.id} style={{background:tipo.id===selectedId?'var(--bg-alt)':undefined}}><td><button className="btn btn-ghost" onClick={()=>seleccionarTipo(tipo.id)}>{tipo.nombre}</button></td><td>{tipo.codigo}</td><td>{etiquetaTipoDocumento(tipo.motor_contenido)}</td><td>{tipo.es_default_para_categoria?'Sí':'No'}</td><td>{tipo.activo?'Activo':'Inactivo'}</td><td>{versionesPorTipo[tipo.id] || '—'}</td><td>{puedeEditar && !tipo.es_default_para_categoria && <button className="btn btn-secondary" onClick={()=>marcarDefault(tipo)} disabled={Boolean(marcandoDefaultId)}>{marcandoDefaultId === tipo.id ? 'Marcando…' : 'Marcar default'}</button>}</td></tr>)}</tbody></table></div>}
    </div></div>}
    {vistaActiva === 'editor' && !tipoSeleccionadoReal && <div className="alert alert-info">Selecciona un tipo de documento desde el Listado para editarlo.</div>}
    {vistaActiva === 'editor' && tipoSeleccionado && <div className="card"><div className="card-head"><div><h3>{tipoSeleccionado.nombre}</h3><div className="text-muted">{publicada ? `Vigente: versión ${publicada.version}` : 'Sin versión publicada'}</div></div><div className="row" style={{gap:8}}>{historial.length > 0 && <button className="btn btn-ghost" onClick={()=>setMostrarHistorial(value=>!value)}>Ver historial de versiones</button>}{puedeCrear && !draft && <button className="btn btn-secondary" onClick={()=>crearBorrador(publicada)}> {publicada ? 'Editar: crear borrador' : 'Crear borrador'} </button>}{draft && puedeEditar && <><button type="button" className="btn btn-ghost" onClick={descartarBorrador} disabled={descartando}>{descartando ? 'Descartando…' : 'Descartar borrador'}</button><button className="btn btn-primary" onClick={publicar} disabled={publicando || descartando}>{publicando ? 'Publicando…' : `Publicar v${draft.version}`}</button></>}</div></div><div className="card-body">
