...
export function CatalogoDocumentosCondiciones({ active }) {
  ...
  const guardarSegmento = async segment => {
    if (!draft) return;
    try {
      const sb = await getSupabaseClient();
      const payload = { condiciones_generales_id:draft.id, titulo:segment.titulo || '', contenido_json:normalizeRichTextDocument(segment.contenido_json), contenido_texto_plano:segment.contenido_texto_plano || '', orden:segment.orden, activo:true };
  ...
  const publicar = async () => {
    if (!draft) return;
    try {
      for (const segmento of segmentos) await guardarSegmento(segmento);
      const sb = await getSupabaseClient();
